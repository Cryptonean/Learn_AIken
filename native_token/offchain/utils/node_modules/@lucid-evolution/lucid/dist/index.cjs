...
var complete = (config, options = {}) => import_effect17.Effect.gen(function* () {
  ...
  const hasScriptExecutions = config.scripts.size > 0;
...
var selectionAndEvaluation = (config, walletInputs, changeAddress, coinSelection, localUPLCEval, script_calculation) => import_effect17.Effect.gen(function* () {
  const availableInputs = import_effect17.Array.differenceWith(import_utils11.isEqualUTxO)(
    walletInputs,
    config.collectedInputs
  );
  const inputsToAdd = coinSelection !== false ? yield* doCoinSelection(config, availableInputs, script_calculation) : [];
  if (import_effect17.Array.isEmptyArray(inputsToAdd) && script_calculation) return;
  let estimatedFee = 0n;
...
var doCoinSelection = (config, availableInputs, script_calculation) => import_effect17.Effect.gen(function* () {
  const estimatedFee = {
...
var recursive = (inputs, requiredAssets, coinsPerUtxoByte, externalAssets = {}, error) => import_effect17.Effect.gen(function* () {
  let selected = [];
